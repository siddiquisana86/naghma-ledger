...
- **Editing/deleting past entries**: available in the app — each entry in the list has an edit (pencil) and delete (trash) action when signed in. Deleting actually removes the row from the Sheet (shifting later rows up), the same as a manual delete; there's a confirm dialog but no undo beyond the Sheet's own version history. Corrections can still be made directly in the Google Sheet too, if preferred.
